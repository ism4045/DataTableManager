...
struct FindBySheetName_Functer
{
    ...
    bool operator()(const TWeakObjectPtr<UStruct>& Obj) const
    {
...
private:
    // Create Table Folder Path Selector
    TSharedRef<SWidget> CreateFolderPathModifier(const FString& InTitleStr, const FString& HintTextValue, const FString& TextBlockValue, TSharedPtr<SEditableTextBox>& InWidgetPtr, void(SDataTableManager::* InFunc_ETB)(const FText&), FReply(SDataTableManager::* InFunc_Btn)());
    TSharedRef<SWidget> CreateFolderPathTitle(const FString& InTitleStr);
    TSharedRef<SWidget> CreateFolderPathTextBox(const FString& HintTextValue, const FString& TextBlockValue, TSharedPtr<SEditableTextBox>& InWidgetPtr, void(SDataTableManager::* InFunc)(const FText&));
    TSharedRef<SWidget> CreateFolderBrowserButton(FReply(SDataTableManager::* InFunc)());
    FReply ExcelFolderBrowserBtnClicked();
    FReply CSVFolderBrowserBtnClicked();
    FReply StructFolderBrowserBtnClicked();
    FReply AssetFolderBrowserBtnClicked();
    FString OpenBrowserAndGetPath(const FString& InPath = "");
    void OnExcelFolderPathTextChanged(const FText& NewText);
    void OnCSVFolderPathTextChanged(const FText& NewText);
    void OnStructFolderPathTextChanged(const FText& NewText);
    void OnAssetFolderPathTextChanged(const FText& NewText);
    ...
    //Entire CheckBox Control
    void OnCheckStateChanged(ECheckBoxState NewState);

    // Refresh ListView Data
    void RefreshSheetState();
    void RefreshCSVState();
...
private:
    ...
    TSharedPtr<SEditableTextBox> ExcelFolderPathWidget;
    TSharedPtr<SEditableTextBox> CSVFolderPathWidget;
    TSharedPtr<SEditableTextBox> StructFolderPathWidget;
    TSharedPtr<SEditableTextBox> AssetFolderPathWidget;
